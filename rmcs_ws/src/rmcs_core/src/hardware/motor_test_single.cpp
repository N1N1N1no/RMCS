...
public:
    MotorTestSingle()
        ...
        , command_component_(
              create_partner_component<MotorTestSingleCommand>(
    ...
    MotorTestSingle(const MotorTestSingle&) = delete;
    MotorTestSingle& operator=(const MotorTestSingle&) = delete;
    MotorTestSingle(MotorTestSingle&&) = delete;
    MotorTestSingle& operator=(MotorTestSingle&&) = delete;

    ~MotorTestSingle() override = default;
...
private:
    class MotorTestSingleCommand : public rmcs_executor::Component {
    public:
        explicit MotorTestSingleCommand(MotorTestSingle& motor_test)
            : motor_test_(motor_test) {}

        void update() override { motor_test_.command_update(); }

        MotorTestSingle& motor_test_;
    };
    std::shared_ptr<MotorTestSingleCommand> command_component_;

    struct MotorBoard final : librmcs::board::CBoard::Callback {
        explicit MotorBoard(
            MotorTestSingle& motor_test, MotorTestSingleCommand& motor_test_command,
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::hardware::MotorTestSingle, rmcs_executor::Component)
